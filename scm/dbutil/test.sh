...
awk -v sdb=$source_db -v ymd=$ymd '{print "drop table if exists " sdb "_" $0 "_" ymd ";"}' $tbllist >$dtsql
...
grep hdfs $sclist | cut -d "'" -f2 |awk -v tgt=${target_uri} -v sdb=${source_db} -v ymd=${ymd} -F '/' '{print "hdfs dfs -cp -f " $0 "/* " tgt "/" sdb "_" $NF "_" ymd}' >$hdfscopysh
...
#	./run_db_tbls_sql.sh ${target_db} $m5list "drop table if exists " 
