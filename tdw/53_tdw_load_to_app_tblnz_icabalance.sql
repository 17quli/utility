...
cast(fdtmlastclosingbalancedate as TIMESTAMP) as fdtmlastclosingbalancedate,
cast(fdtmlastupdate as TIMESTAMP) as fdtmlastupdate,
cast(fcuricabalance as DECIMAL(19,4)) as fcuricabalance,
...
select '${var:app_tbl_name}','${var:env}',${var:batch_number},'${var:tdw_tbl_name}',count(distinct flngaccountkey)  as distinct_keys_count FROM  ${var:target_db}.${var:app_tbl_name};
upsert into ${var:source_db}.tdw_validation(raw_table_name,environment,batch_number,tdw_table_name,tdw_multiple_key_count)
select '${var:app_tbl_name}','${var:env}',${var:batch_number},'${var:tdw_tbl_name}', count(1) from (select flngaccountkey,count(1) FROM  ${var:target_db}.${var:app_tbl_name} group by flngaccountkey having count(1)>1) z;
