#!/bin/bash
#20200908	:comment out benchmark.sh to reduce workload on the platform
...
pidlist=./pid_$$.lst
>$pidlist
#/home/ird0qal/dbutil/benchmark/benchmark.sh &
./cmd.sh zealandia cmd_top &
echo $! >>$pidlist
./cmd.sh zealandia cmd_df &
echo $! >>$pidlist
./cmd.sh zealandia cmd_free &
echo $! >>$pidlist
wait <$pidlist
./fmt.sh
#./cmd.sh edgenode cmd_5minute
