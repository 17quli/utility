...
scripts to create  
  dev_raw_full_access view for a "fast Copied" source table 
INPUTS:
 surce table name  and schema

OUTPUT:
   "
     "/sso/biconfig/940/Lev1/Logs/Validation_outputs/create_impala_full_access_views_&sysuserid..txt"
...
%macro  create_fcp_fullaccess_views(schema=,Object_Name= , Target_table_name= ,pattern=  ,table_num=)/ minoperator mindelimiter=','; 
%put &=schema.;
...
%do;
   *filename tmpfile1 "/sso/biconfig/940/Lev1/Logs/Validation_outputs/create_impala_tables.txt" lrecl=256 new; 
   filename tmpfile2 "/sso/biconfig/940/Lev1/Logs/Validation_outputs/create_impala_full_access_views_&sysuserid..txt" lrecl=256 new;
    TITLE;
    FOOTNOTE;
%end;
%else 
%do;
   *filename tmpfile1 "/sso/biconfig/940/Lev1/Logs/Validation_outputs/create_impala_tables.txt" lrecl=256 ; 
   filename tmpfile2 "/sso/biconfig/940/Lev1/Logs/Validation_outputs/create_impala_full_access_views_&sysuserid..txt" lrecl=256 ; 
%end;
...
data _null_;            
   ...
   PUT  " FROM ${source_db=Enter the source database name here}.&Object_Name. z" ; 
   PUT ";";
run;
 quit;
 title; 
