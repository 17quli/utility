%macro metadata_check_timestamp / minoperator mindelimiter=',';
/*
    Name:                           metadata_check_timestamp
    ...
    Remarks:                        Walks through metadata folders, picking out jobs' Impala targets,
                                    looking for timestamp discrepancies
...
*/
%local i dsid rc source_library source_table _target_library target_table _source_memtype sas_table log;
...
Keys:
 ...
 do j = 1 by 1 while(metadata_getnasn(step_uri, 'PropertySets', j, substep_uri1) > 0);
    ...
    do k = 1 by 1 while(metadata_getnasn(substep_uri1, 'Properties', k, substep_uri2) > 0);
       ...
       if not(value1 = 'ep_key_cols' and value2 =: 'Natural') then
          continue;
...
%do i = 1 %to &nobs;
    ...
    %check_timestamp(table=&target_name, key=&key_columns, schema=&schema);
    %end;
