...
Purpose: Script to restore the LCF objects to pre release 2.1 status.

Instructions:   There are several types of objects to be backed up prior to release 2.1 deployment of LCF.
...
-- Backup the currernt tables prior to restore

SELECT TABLE_NAME FROM USER_TABLES;


-- DIP_CONFIG
RENAME DIP_CONFIG TO DIP_CONFIG_POST21_BKP_&today ;


--SYSTEM_SCHEMA
RENAME SYSTEM_SCHEMA TO SYSTEM_SCHEMA_POST21_BKP_&today;


--OBJECT
RENAME OBJECT TO OBJECT_POST21_BKP_&today ;


--OBJECT_HIST
RENAME OBJECT_HIST TO OBJECT_HIST_POST21_BKP_&today;



--EXTRACT_OBJECT
RENAME EXTRACT_OBJECT TO EXTRACT_OBJECT_POST21_BKP_&today;


--OBJECT_RUN
RENAME OBJECT_RUN TO OBJECT_RUN_POST21_BKP_&today ;

--SYSTEM_SCHEMA_HIST
RENAME SYSTEM_SCHEMA TO SYSTEM_SCHEMA_HIST_POST21_BKP_&today ;



--DIP_OBJECT_RUN
RENAME DIP_OBJECT_RUN TO DIP_OBJECT_RUN_POST21_BKP_&today;

 
-- DIP_CONFIG
RENAME TABLE DIP_CONIG TO DIP_CONFIG&today;



-- No differences to these tables between pre-2.1 and post 2.1
...
/*-------------------------------------------------------------------------------------------------------*/
-- Restore the tables using pre 2.1 backups. &last_backup is when the backup was last done prior to release 2.1.


-- DIP_CONFIG
CREATE TABLE DIP_CONFIG 
AS SELECT * 
FROM DIP_CONFIG_PRE21_BKP_&last_backup;

--SYSTEM_SCHEMA
CREATE TABLE SYSTEM_SCHEMA
AS SELECT * 
FROM SYSTEM_SCHEMA_PRE21_BKP_&last_backup;

--OBJECT
CREATE TABLE OBJECT
AS SELECT * 
FROM OBJECT_PRE21_BKP_&last_backup 

--OBJECT_HIST
CREATE TABLE OBJECT_HIST
AS SELECT * 
FROM OBJECT_HIST_PRE21_BKP_&last_backup;


--EXTRACT_OBJECT
CREATE TABLE EXTRACT_OBJECT 
AS SELECT * 
FROM EXTRACT_OBJECT_PRE21_BKP_&last_backup;

--OBJECT_RUN
CREATE TABLE OBJECT_RUN 
AS SELECT * 
FROM OBJECT_RUN_PRE21_BKP_&last_backup ;


--SYSTEM_SCHEMA_HIST
CREATE TABLE SYSTEM_SCHEMA_HIST
AS SELECT * 
FROM SYSTEM_SCHEMA_HIST_PRE21_BKP_&last_backup;


--DIP_OBJECT_RUN
CREATE TABLE  DIP_OBJECT_RUN 
AS SELECT * 
FROM DIP_OBJECT_RUN_PRE21_BKP_&last_backup;


-- No differences to these tables between pre-2.1 and post 2.1. No need to restore.
...
-- At this point decide whether to recreate the sequences or not using the sequence definitions below. After release 2.1
--   sequences would have advanced. 
--  When you run dbmsmetadata.get_ddl  you get  the following as an example.
--   CREATE SEQUENCE  "LCF"."SEQ_DIP_OBJECT_RUN_KEY"  MINVALUE 1 MAXVALUE 9999999999999999999999999999 INCREMENT BY 1 START WITH 1521 CACHE 20 NOORDER  NOCYCLE  NOKEEP  NOSCALE  GLOBAL "
...
--Drop the existing triggers and create them using pre-21 logic 
 DROP TRIGGER DOMAIN_BRIDGE_KEY;
...
DROP  TRIGGER TRG_DIP_OBJECT_RUN_KEY;
...
DROP TRIGGER TRG_SYSTEM_SCHEMA_KEY;
