...
write-host "Cleaning up the Jobs folder..."
$dummy_cmd = "& """+$AppProps.puttyFolder+"\plink.exe"" -hostkey "+$AppProps.importHostKey+" $user@"+$AppProps.importComputeHost+" -batch ""touch "+$AppProps.serverLvDir+"/SASApp/SASEnvironment/SASCode/Jobs/dummy.sas""";
iex $dummy_cmd;
if ( $LASTEXITCODE -ne 0 ){ exit; }
$cleanup_jobs_cmd = "& """+$AppProps.puttyFolder+"\plink.exe"" -hostkey "+$AppProps.importHostKey+" $user@"+$AppProps.importComputeHost+" -batch rm "+$AppProps.serverLvDir+"/SASApp/SASEnvironment/SASCode/Jobs/*.sas -f";
...
write-host "Cleaning up the the SASMacro folder...."
$dummy_cmd = "& """+$AppProps.puttyFolder+"\plink.exe"" -hostkey "+$AppProps.importHostKey+" $user@"+$AppProps.importComputeHost+" -batch ""touch "+$AppProps.serverLvDir+"/SASApp/SASEnvironment/SASMacro/dummy.sas""";
iex $dummy_cmd;
if ( $LASTEXITCODE -ne 0 ){ exit; }
$cleanup_cmd = "& """+$AppProps.puttyFolder+"\plink.exe"" -hostkey "+$AppProps.importHostKey+" $user@"+$AppProps.importComputeHost+" -batch rm "+$AppProps.serverLvDir+"/SASApp/SASEnvironment/SASMacro/*.sas -f";
iex $cleanup_cmd;
if ( $LASTEXITCODE -ne 0 ){ exit; }

write-host "Uploading Macros to SASApp....."
$upload_cmd = "& """+$AppProps.puttyFolder+"\pscp.exe"" -hostkey "+$AppProps.importHostKey+" -r -batch "+$AppProps.importPath+$AppProps.macroFolder+"\ $user@"+$AppProps.importComputeHost+":"+$AppProps.serverLvDir+"/SASApp/SASEnvironment/SASMacro/";
iex $upload_cmd;
if ( $LASTEXITCODE -ne 0 ){ exit; }


write-host "Executing update enviornment value job...."
$update_cmd = "& """+$AppProps.puttyFolder+"\plink.exe"" -hostkey "+$AppProps.importHostKey+" $user@"+$AppProps.importComputeHost+" -batch """+$AppProps.serverLvDir+"/SASApp/BatchServer/sasbatch.sh -noprint -log "+$AppProps.serverLvDir+"/Logs/Change_Hadoop_environment_#Y.#m.#d_#H.#M.#s.log  -batch -noterminal -logparm 'rollover=session'  -sysin "+$AppProps.serverLvDir+"/SASApp/SASEnvironment/SASCode/Jobs/Change_Hadoop_environment_"+$AppProps.targetEnv+".sas""";
iex $update_cmd;
...
if ( $refresh_workflow -ne 0 )
{
    ...
    $refresh_wokflow_def_cmd = "& """+$AppProps.puttyFolder+"\plink.exe"" -hostkey "+$AppProps.importHostKey+" $user@"+$AppProps.importComputeHost+" -batch """+$AppProps.serverLvDir+"/SASApp/BatchServer/sasbatch.sh -noprint -log "+$AppProps.serverLvDir+"/Logs/LCF_WORKFLOW_JOB_#Y.#m.#d_#H.#M.#s.log  -batch -noterminal -logparm 'rollover=session'  -sysin "+$AppProps.serverLvDir+"/SASApp/SASEnvironment/SASCode/Jobs/LCF_WORKFLOW_JOB.sas""";
...
write-host "Creating a tmp folder in users's directory..."
$tmp_cmd = "& """+$AppProps.puttyFolder+"\plink.exe"" -hostkey "+$AppProps.importHostKey+" $user@"+$AppProps.importComputeHost+" -batch mkdir -p ~/tmp_deploy";
iex $tmp_cmd;

write-host "Redploying all the jobs..."
$env_cmd = "& """+$AppProps.puttyFolder+"\plink.exe"" -hostkey "+$AppProps.importHostKey+" $user@"+$AppProps.importComputeHost+" -batch """+$AppProps.batchDeployCmdPath+"/DeployJobs -deploytype REDEPLOY -host "+$AppProps.importHost+" -port "+$AppProps.importPort+" -user $user -password $password -appservername SASApp -metarepository Foundation -sourcedir ~/tmp_deploy  -objects '"+$AppProps.importProjectFolder+"' -recursive""";
iex $env_cmd;
if ( $LASTEXITCODE -ne 0 ){ exit; }

write-host "Cleaning up temporary source jobs..."
$cleanup_tmp_cmd = "& """+$AppProps.puttyFolder+"\plink.exe"" -hostkey "+$AppProps.importHostKey+" $user@"+$AppProps.importComputeHost+" -batch rm -rf ~/tmp_deploy";
iex $cleanup_tmp_cmd;
