...
then
#default to full
	sqoop import --connect jdbc:oracle:thin:@//ird74au.vsp.sas.com:6660/irdtst20.vsp.sas.com -m 1 --table $source_table --username lcf --password gEjmEtd2x --target-dir /env/2_test/hive/dw/$target_table --delete-target-dir --fields-terminated-by '\001' --bindir /home/ird0qal/sqoop/stg --temporary-rootdir /user/ird0qal/temporary_root

else
	lv=`./run_db_sqlb.sh dev_raw "select max($check_column) from $target_table"`
	sqoop import --connect jdbc:oracle:thin:@//ird74au.vsp.sas.com:6660/irdtst20.vsp.sas.com -m 1 --table $source_table --username lcf --password gEjmEtd2x --target-dir /env/2_test/hive/dw/$target_table --fields-terminated-by '\001' --incremental lastmodified --check-column START_TIME --last-value "$lv" --merge-key $merge_key --bindir /home/ird0qal/sqoop/stg --temporary-rootdir /user/ird0qal/temporary_root 
fi
