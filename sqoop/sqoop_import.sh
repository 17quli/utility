...
test $# -eq 7 ||test $# -eq 8|| error_exit "Wrong # of arguments! Type `basename $0` <source_db> <source_tbl> <target_db> <target_tbl> <change_capture_column> <# of mappers> <split_by>  <excluded_columns>"
test "$tbl_mappers" -eq "$tbl_mappers" 2>/dev/null|| error_exit "table mappers is not a valid number:tbl_mappers=$tbl_mappers"
...
sqoop_import=./stg/sqoop_import_${target_tbl}_${ts}.cmd

cleanup(){
rm -f "$sql_insert" "$sqoop_import"
#rm -f "$sql_insert" "$sqoop_import"
}
...
#	test -n "$sqoop_table_location" || error_exit "sqoop table ${source_db}.${source_tbl} hdfs location is empty" status "'failed $step'"
	...
	then
		test "$lv" = "NULL" && lv="to_timestamp('1800-1-1 00:00:00.000000000','YYYY-MM-DD HH24:MI:SS.ff9')"
		where_clause="$cdc_col>$lv"
...
cat >$sqoop_import <<EOF
sqoop import  -Dorg.apache.sqoop.splitter.allow_text_splitter=true -D mapred.child.java.opts="-Djava.security.egd=file:/dev/../dev/urandom" --verbose --connect $connect -m $tbl_mappers --table $source_tbl --username $username --password-file $password_file  --hive-import --hive-database $target_db --hive-table $sqoop_tbl --fields-terminated-by '\0001' --hive-delims-replacement ' ' --null-string '\\\\N' --null-non-string '\\\\N' --bindir=$bindir --delete-compile-dir --outdir "$stgdir" --target-dir $tbl_target_dir --delete-target-dir --temporary-rootdir $target_dir $map_column_hive $where_clause  $split_by --optionally-enclosed-by '\"' --escaped-by \\\\
EOF

cat $sqoop_import

sh $sqoop_import || error_exit "failed to sqoop import from ${source_db}.${source_tbl} to ${target_db}.${target_tbl}:return_code=$?" status "'failed $step'"

#if [ -n "$map_column_hive" ]
#then
#	if [ -n "$where_clause" ]
#	then
#		sqoop import  -Dorg.apache.sqoop.splitter.allow_text_splitter=true -D mapred.child.java.opts="-Djava.security.egd=file:/dev/../dev/urandom" --connect "$connect" -m $tbl_mappers --table $source_tbl --username $username --password-file $password_file  --hive-import --hive-database $target_db --hive-table $sqoop_tbl --fields-terminated-by '\0001' --hive-delims-replacement ' ' --null-string '\\N' --null-non-string '\\N' --bindir=$bindir --delete-compile-dir --outdir "$stgdir" --target-dir $tbl_target_dir --delete-target-dir --temporary-rootdir $target_dir --map-column-hive "$map_column_hive" --where "$where_clause"  --optionally-enclosed-by '\"' --escaped-by \\
#	else
#		sqoop import  -Dorg.apache.sqoop.splitter.allow_text_splitter=true -D mapred.child.java.opts="-Djava.security.egd=file:/dev/../dev/urandom" --connect "$connect" -m $tbl_mappers --table $source_tbl --username $username --password-file $password_file  --hive-import --hive-database $target_db --hive-table $sqoop_tbl --fields-terminated-by '\0001' --hive-delims-replacement ' ' --null-string '\\N' --null-non-string '\\N' --bindir=$bindir --delete-compile-dir --outdir "$stgdir" --target-dir $tbl_target_dir --delete-target-dir --temporary-rootdir $target_dir --map-column-hive "$map_column_hive"  --optionally-enclosed-by '\"' --escaped-by \\
#	fi
#else
#	if [ -n "$where_clause" ]
#        then
#                sqoop import  -Dorg.apache.sqoop.splitter.allow_text_splitter=true -D mapred.child.java.opts="-Djava.security.egd=file:/dev/../dev/urandom" --connect "$connect" -m $tbl_mappers --table $source_tbl --username $username --password-file $password_file  --hive-import --hive-database $target_db --hive-table $sqoop_tbl --fields-terminated-by '\0001' --hive-delims-replacement ' ' --null-string '\\N' --null-non-string '\\N' --bindir=$bindir --delete-compile-dir --outdir "$stgdir" --target-dir $tbl_target_dir --delete-target-dir --temporary-rootdir $target_dir  --where "$where_clause"  --optionally-enclosed-by '\"' --escaped-by \\
#        else
#                sqoop import  -Dorg.apache.sqoop.splitter.allow_text_splitter=true -D mapred.child.java.opts="-Djava.security.egd=file:/dev/../dev/urandom" --connect "$connect" -m $tbl_mappers --table $source_tbl --username $username --password-file $password_file  --hive-import --hive-database $target_db --hive-table $sqoop_tbl --fields-terminated-by '\0001' --hive-delims-replacement ' ' --null-string '\\N' --null-non-string '\\N' --bindir=$bindir --delete-compile-dir --outdir "$stgdir" --target-dir $tbl_target_dir --delete-target-dir --temporary-rootdir $target_dir   --optionally-enclosed-by '\"' --escaped-by '\\'
#	fi
#fi
...
#keep the sqoop table for sizing and benchmarking
#./run_sqlb.sh "drop table if exists ${target_db}.${sqoop_tbl} purge;"
