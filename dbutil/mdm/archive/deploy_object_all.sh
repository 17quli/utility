...
case "$deploy_type" in
	table|full_view|restricted_view) echo `date +"$dfmt"`"start ${0}" |tee -a $body;;
...
impala-shell --quiet -i ird10139au.vsp.sas.com:25003  -Q "sync_ddl=1" -f ${sql_file}  --var="db_env=${db_env}"  --var="db_layer=${db_layer}" --var="deploy_type=${deploy_type}" -k --ssl --ca_cert=/sso/hadoop/cloudera-certs/CACerts/sas-hadoop-truststore.pem |tee -a ${stg_file}
sed -i 's/^"//g' $stg_file
sed -i 's/"$//g' $stg_file
