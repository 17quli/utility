...
then
	...
	echo "type $0 <source_db> <target_db> <prefix> <suffix> <tbl_list>" 
	exit 1
fi

echo `date +"$dfmt"`"start ${0} ${source_db} ${target_db} prefix $prefix suffix $suffix tbllist $tbllist" |tee -a $body
...
sed -i -e "s/^/$prefix/g" $tbllist
sed -i -e "s/$/$suffix/g" $tbllist
