...
then
	...
	echo "type $0 <source_db.source_table> <target_db>"|tee -a $logfile
	exit 1
fi
...
then
	echo "<source> should be like <source_db>.<source_table>"|tee -a $logfile
	exit 2
fi
...
if [ $? -ne 0 ]
then
	echo "execute ${sql} failed" |tee -a $logfile
	exit 5
fi
...
then
	echo "${source} does not exist"|tee -a $logfile
	exit 6
fi
...
then
	echo "execute describe database ${target_db} failed"|tee -a $logfile
	exit 7
fi
...
echo "dropping table ${target}......"|tee -a $logfile
impala-shell -B -i ird26au.vsp.sas.com:25003  -q "drop table if exists ${target};" -k --ssl --ca_cert=/sso/hadoop/cloudera-certs/CACerts/DigiAndSAS-CA.pem 2>&1|tee -a $logfile

sleep 5

echo "creating table ${target}......"|tee -a $logfile
impala-shell -B -i ird26au.vsp.sas.com:25003  -q "create table ${target} like ${source};" -k --ssl --ca_cert=/sso/hadoop/cloudera-certs/CACerts/DigiAndSAS-CA.pem 2>&1|tee -a $logfile
...
impala-shell -i ird26au.vsp.sas.com:25003  -q "${sql}" -k --ssl --ca_cert=/sso/hadoop/cloudera-certs/CACerts/DigiAndSAS-CA.pem 2>&1|tee -a $logfile
