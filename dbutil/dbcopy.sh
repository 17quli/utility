#!/bin/bash
#20200901:	make create table paralle with hdfs copy
#20201221:	wait for user's input before exiting copy to production database
#set -x 
...
if [ $target_env = "prod" ] && [ $target_db != "prod_dw" ]
then
       # echo "$0 does not support copy to production database"
	read -p "$0 does not support copy to production database,press any key to exit!"
	exit 2
fi
