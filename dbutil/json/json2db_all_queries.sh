...
#set -x
cd `dirname $0`
json2txtcmd=./json2txt_all_queries.sh
...
do
	...
	./run_db_sqlb.sh dev_raw "truncate table mquery_all_queries_stg" 
#	if [[ "$peak_hours" =~ "$hour" ]]
#        then
...
#	fi
	...
	#	wget --user ird0qal --password `cat .pass` -O ${jsonfile} https://ird01au.vsp.sas.com:7183/api/v19/clusters/IRD-PRD-01/services/impala/impalaQueries?from=${fromts}\&to=${fromtsnext}\&limit=1000\&offset=${offset} 
		wget --user ird0qal --password `cat .pass` --no-check-certificate -O $jsonfile "$cmurl"?from=${fromts}\&to=${fromtsnext}\&limit=1000\&offset=${offset} 
	...
	wait <$pidlist
        ...
        ./run_db_sqlb.sh dev_raw "refresh mquery_all_queries_stg;upsert into mquery_all_queries select * from mquery_all_queries_stg;"
done
