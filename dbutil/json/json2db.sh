...
#	fi
	...
	#	wget --user ird0qal --password `cat .pass` -O ${jsonfile} https://ird01au.vsp.sas.com:7183/api/v19/clusters/IRD-PRD-01/services/impala/impalaQueries?from=${fromts}\&to=${fromtsnext}\&limit=1000\&offset=${offset} 
		wget --user ird0qal --password `cat .pass` --no-check-certificate -O $jsonfile "$cmurl"?from=${fromts}\&to=${fromtsnext}\&limit=1000\&filter=\(query_duration\>${query_duration}s\)\&offset=${offset} 
#		wget --user ird0qallocal --password '88UTHmnb$$'  --no-check-certificate -O $jsonfile "$cmurl"?from=${fromts}\&to=${fromtsnext}\&limit=1000\&filter=\(query_duration\>${query_duration}s\)\&offset=${offset} 
