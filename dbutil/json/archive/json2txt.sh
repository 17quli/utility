...
if [ $# -ne 1 ]
then
        echo "type $0 <json_file>"|tee -a $logfile
        exit 1
fi

if [ ! -f $json ]
then
        echo "${json} is not a valid file"|tee -a $logfile
        exit 2
fi
...
if [ ! -s $jsonstg ]
then
	echo "empty json file!" |tee -a $logfile
	exit 3
fi

linecount=`wc -l <$jsonstg`
queries=`grep queryId $jsonstg|wc -l`
echo `date +"$dfmt"`"start converting $queries queries from json to text file `basename $txtstg`" |tee -a $logfile|tee -a $body
...
do
	...
	#set -- $line
	name=$(echo $line |awk -F " : " '{gsub("\"","");print $1}')
	value=$(echo $line |awk -F " : " '{gsub("\"","");print $2}')
	if [ "$name" = "queryId" ] 
	then
		if [ "${q[queryId]}" != "" ]
		then
			#write to txtstg
			#echo "writing to $txtstg  queryId=${q[queryId]}" |tee -a $logfile
			print_query
...
echo `date +"$dfmt"`"truncate staging table" |tee -a $logfile
./run_db_sqlb.sh dev_raw "truncate table query_stg"
echo `date +"$dfmt"`"start uploading text file to HDFS" |tee -a $logfile
hdfs dfs -put -f $txtstg /env/1_dev/hive/raw/query_stg |tee -a $logfile
echo `date +"$dfmt"`"load from staging into query" |tee -a $logfile
