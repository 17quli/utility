...
if [ "$json_record_first_column" = "" ]
then
#1st colun in cdh6 is rowsProduced
	json_record_first_column=rowsProduced
fi
...
#	first column indicates a new query: changed from queryId in cdh5 to rowsProduced in cdh6
#	if [ "$name" = "queryId" ] 
	if [ "$name" = "$json_record_first_column" ] 
	then
		if [ "${q[queryId]}" != "" ]
		then
			#write to txtstg
...
#	./run_db_sqlb.sh dev_raw "truncate table query_stg"
	...
	hdfs dfs -put -f $txtstg /env/1_dev/hive/raw/mquery_all_queries_stg 
